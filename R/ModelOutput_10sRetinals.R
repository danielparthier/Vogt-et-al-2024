...
ModelDirectory <- "Output/20240611_10sRetinals/"

Model <- readRDS(file = paste0(ModelDirectory,"Modelout.rds"))
RetinalColours <- c("JellyOp-mScarlet.9-cis"="blue","JellyOp-mScarlet.11-cis"="green","JellyOp-mScarlet.13-cis"="purple")
dir.create(path = paste0(ModelDirectory,"Plots"), showWarnings = F)
dir.create(path = paste0(ModelDirectory,"Tables"), showWarnings = F)

AssayList <- LoadData("jellyRetinalsOnly", illumination_range = c(10000, 10000))
...
LambdaMaxPlateGroup[,GroupName:=factor(x=GroupName, levels = c("JellyOp-mScarlet.9-cis","JellyOp-mScarlet.11-cis", "JellyOp-mScarlet.13-cis")),]
...
LambdaMaxPlateGroup[,.(PeakLocation=mean(PeakLocation)),by=.(GroupName, Sample)][,calculate_HDI(PeakLocation),by=GroupName]
...
(RetinalPeaksPlatePlot <- ggplot(LambdaMaxPlateGroup, aes(x=PeakLocation, y=Plate, group=interaction(GroupName,Plate), fill=GroupName, colour=GroupName))+
  ...
  scale_colour_manual(name = "Retinal", values=RetinalColours, aesthetics=c("colour", "fill"))+
  theme_classic()+
  theme(strip.background = element_blank(), strip.text = element_text(size=12), axis.ticks.length = unit(x=AxisTickLength, units="pt")))
ggsave(paste0(ModelDirectory,"Plots/RetinalPeaksPlatePlot.pdf"), RetinalPeaksPlatePlot, width = 10, height = 10)
saveRDS(RetinalPeaksPlatePlot, file = paste0(ModelDirectory,"Plots/RetinalPeaksPlatePlot.rds"))  

(RetinalPeaksPlot <- ggplot(LambdaMaxPlateGroup[,.(PeakLocation=mean(PeakLocation)),by=.(GroupName, Sample)], aes(x=PeakLocation, group=GroupName, fill=GroupName, colour=GroupName))+
  stat_slab(height=1.5, colour="black", alpha=0.6)+
  ...
  theme_classic()+
  theme(strip.background = element_blank(), strip.text = element_text(size=12), axis.ticks.length = unit(x=AxisTickLength, units="pt")))
ggsave(paste0(ModelDirectory,"Plots/RetinalPeaksPlot.pdf"), RetinalPeaksPlot, width = 10, height = 10)
saveRDS(RetinalPeaksPlot, file = paste0(ModelDirectory,"Plots/RetinalPeaksPlot.rds"))

KableStringRetinalPeaks <- gsub(pattern = "NA", replacement = "  ", x = knitr::kable(LambdaMaxPlateGroup[,.(PeakLocation=mean(PeakLocation)),by=.(GroupName, Sample)][,calculate_HDI(PeakLocation),by=GroupName][,-2,], digits = c(0,1,1,1,1,1,1,1)))
KableStringRetinalPeakDiff <- gsub(pattern = "NA", replacement = "  ", x = knitr::kable(GroupComparisonHDI[,-2,], digits = c(0,1,1,1,1,1,1,1)))
KableStringRetinalPeakDiffProb <- gsub(pattern = "NA", replacement = "  ", x = knitr::kable(GroupComparisonProb, digits = c(0,6)))

RmdString <- paste0(ModelDirectory,"Tables/Summary.Rmd")
cat(paste0("## Retina Comparison\n"),
    "\n### Activity Peaks (in nm)",
    KableStringRetinalPeaks,
    "\n### Differences in Activity Peaks (in nm)", 
    KableStringRetinalPeakDiff,
    "\n### Difference Probabilities (from 0 nm shift)", 
    KableStringRetinalPeakDiffProb,
    sep="\n", file=RmdString)
...
### calculate actual avtivity from I50 and activityMax
...
(GroupActivityWithDarkSpectrum <- ggplot(data=Activity_Group_HDI, aes(y=Mean, x=Wavelength, group=Group, colour=GroupName))+
  geom_ribbon(aes(ymin=HDI95.CI_low, ymax=HDI95.CI_high, fill=GroupName,group=Group,x=Wavelength),alpha=0.1, colour=NaN)+
  geom_line()+
   scale_colour_manual(name = "Retinal", values=RetinalColours, aesthetics=c("colour", "fill"))+
  ...
  geom_errorbar(data=AssayData[,.(Mean=mean(Activity), SD=sd(Activity), SEM=sd(Activity)/sqrt(.N)),by=.(Wavelength, ConstructRetinal)], aes(x=Wavelength, ymin=Mean-SEM, ymax=Mean+SEM, group=ConstructRetinal, colour=ConstructRetinal), width=6, inherit.aes = F)+
  scale_y_continuous(name="luminescence (norm.)", expand=c(0,0), breaks=seq(0,0.2,0.05), limits=c(0,0.2))+
  scale_x_continuous(name = "wavelength in nm",  breaks = seq(300,700,100), minor_breaks = seq(300,700,20), expand=c(0,0), limit=c(360,700))+
  theme_classic()+theme(strip.background = element_blank(), strip.text = element_text(size=12), axis.ticks.length = unit(x=AxisTickLength, units="pt"),legend.position = "inside", legend.position.inside =  c(.4, .11))+
  guides(x = guide_axis(minor.ticks = TRUE)))
...
(GroupActivityCorrectedSpectrum <- ggplot(data=Activity_corrected_Group_HDI, aes(y=Mean, x=Wavelength, group=Group, colour=GroupName))+
  geom_ribbon(aes(ymin=HDI95.CI_low, ymax=HDI95.CI_high, fill=GroupName,group=Group,x=Wavelength),alpha=0.1, colour=NaN)+
  geom_line()+
   scale_colour_manual(name = "Retinal", values=RetinalColours, aesthetics=c("colour", "fill"))+
  geom_point(data=AssayData[,.(Activity=mean(Activity_corrected)),by=.(Wavelength, ConstructRetinal)], aes(x=Wavelength, y=Activity, group=ConstructRetinal, colour=ConstructRetinal), size=3, inherit.aes = F)+
  geom_errorbar(data=AssayData[,.(Mean=mean(Activity_corrected), SD=sd(Activity_corrected), SEM=sd(Activity_corrected)/sqrt(.N)),by=.(Wavelength, ConstructRetinal)], aes(x=Wavelength, ymin=Mean-SEM, ymax=Mean+SEM, group=ConstructRetinal, colour=ConstructRetinal), width=6, inherit.aes = F)+
  scale_y_continuous(name="luminescence (norm.)", expand=c(0,0), breaks=seq(0,0.2,0.05), limits=c(-0.01,0.2))+
