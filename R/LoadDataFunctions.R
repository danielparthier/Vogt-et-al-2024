...
LoadData <- function(DataSet, illumination_range = c(0, Inf)) {
  ...
  suppressMessages(
    {
      if(DataSet=="includeRetinals") {
        DataContainer <- list(
          AssayData = list(
            readxl::read_xlsx(path = "Data/JellyOp light titration.xlsx", skip = 7, col_names = F, n_max = 12),
            readxl::read_xlsx(path = "Data/JellyOp retinal isomers 10s.xlsx", skip = 7, col_names = F, n_max = 12),
            readxl::read_xlsx(path = "Data/bovRho light titration.xlsx", skip = 7, col_names = F, n_max = 12),
            readxl::read_xlsx(path = "Data/bovRho retinal isomers 10s.xlsx", skip = 7, col_names = F, n_max = 12)
          ),
          LoadMetaData = list(
            readxl::read_xlsx(path = "Data/JellyOp light titration.xlsx", col_names = F, n_max = 7),
            readxl::read_xlsx(path = "Data/JellyOp retinal isomers 10s.xlsx", col_names = F, n_max = 7),
            readxl::read_xlsx(path = "Data/bovRho light titration.xlsx", col_names = F, n_max = 7),
            readxl::read_xlsx(path = "Data/bovRho retinal isomers 10s.xlsx", col_names = F, n_max = 7)
          )
        )
      } else if (DataSet=="fullATR" | DataSet=="fullATR10s" | DataSet=="fullATRTrimmed" | DataSet=="fullATRBov10sOnly") {
        DataContainer <- list(
          AssayData = list(
            readxl::read_xlsx(path = "Data/JellyOp light titration.xlsx", skip = 7, col_names = F, n_max = 12),
            readxl::read_xlsx(path = "Data/bovRho light titration.xlsx", skip = 7, col_names = F, n_max = 12)
  ...
  if(DataSet=="fullATR10s") {
    ...
    AssayData <- AssayData[(construct=="JellyOp-mScarlet" & illumination_time <60000)|(construct=="bovRho-mScarlet" & illumination_time==10000),]
  } else if (DataSet == "includeRetinals") {
    AssayData <- AssayData[illumination_time %between% illumination_range,]
  } else if (DataSet == "fullATR") {
    AssayData <- AssayData[!(!(illumination_time %between% c(1000,30000)) & construct=="bovRho-mScarlet"),]
